'''
 ...
 Add-on
	...
	Re-used the code from phoenix ...thanks to phoenix Team 
...
class indexer:
    ...
    def root(self):
        try:
            ...
            dialog.ok('Disclaimer:','All of the content provided on this add-on is totally free and accessible to all on the internet on their various websites. It was simply designed for convenience only. If there are any complaints of broken links or Copyright infringement, please see email address below', 'E-Mail: support-team@cazwall.com')
            url = 'https://goo.gl/GwLn3W'
    ...
    def developer(self):
        try:
            ...
            self.list = self.streams_list('', result=result)
            self.addDirectory(self.list)
            return self.list
        except:
            pass
    ...
    def addSearch(self, url=None):
        try:
            ...
            self.list = [] ; threads = [] 
            for link in links: threads.append(workers.Thread(self.streams_list, link))
            ...
            for i in self.list:
                try:
                    name = ''
                    if not i['vip'] in ['streams TV']: name += '[B]%s[/B] | ' % i['vip'].upper()
                    name += i['name']
                    i.update({'name' : name})
                except:
                    pass
    ...
    def streams_list(self, url, result=None):
        try:
